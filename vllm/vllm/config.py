...
class StreamingConfig:
    ...
    Streaming-LLM enables processing of infinite-length inputs by maintaining
    only a fixed-size KV cache consisting of attention sinks (initial tokens)
    and recent tokens, while discarding intermediate tokens.
    """

    enable_streaming: bool = False
    """Whether to enable Streaming-LLM functionality."""

    start_size: int = 4
    """Number of initial tokens to keep as attention sinks. These tokens
    help maintain attention stability across long sequences."""

    recent_size: int = 512
    """Number of recent tokens to keep in the sliding window. This maintains
    the model's understanding of current context."""

    enable_pos_shift: bool = True
    """Whether to enable position shift for rotary position embeddings.
    ...
    """List of model architectures that support streaming."""
    ...
    def __post_init__(self):
        if self.enable_streaming:
            ...
            if self.start_size + self.recent_size > 32768:
                logger.warning(
                    f"Large cache size ({self.start_size + self.recent_size}) "
                    "may impact performance. Consider reducing start_size or recent_size."
                )

    @property
    def cache_size(self) -> int:
        """Total size of the streaming cache."""
        return self.start_size + self.recent_size

    def is_model_supported(self, model_type: str) -> bool:
        """Check if a model type is supported for streaming."""
        return any(supported in model_type.lower()
                  for supported in self.supported_models)

    def compute_hash(self) -> str:
        """Compute hash for streaming configuration."""
        import hashlib
        factors = [
            self.enable_streaming,
            self.start_size,
            self.recent_size,
            self.enable_pos_shift,
...
class VllmConfig:
    ...
    def compute_hash(self) -> str:
        ...
        if self.streaming_config:
            vllm_factors.append(self.streaming_config.compute_hash())
        else:
            vllm_factors.append("None")
