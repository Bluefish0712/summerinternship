...
from vllm.config import (BlockSize, CacheConfig, CacheDType, CompilationConfig,
                         ...
                         StreamingConfig, TaskOption, TokenizerPoolConfig,
...
class EngineArgs:
    ...
    # Streaming-LLM specific arguments
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # Streaming-LLM arguments
        parser.add_argument(
            "--enable-streaming-llm",
            action="store_true",
            default=EngineArgs.enable_streaming_llm,
            help="Enable Streaming-LLM for processing infinite-length inputs "
            "with fixed memory usage. This maintains only attention sinks "
            "(initial tokens) and recent tokens while discarding intermediate tokens.")
        parser.add_argument(
            "--streaming-start-size",
            type=int,
            default=EngineArgs.streaming_start_size,
            help="Number of initial tokens to keep as attention sinks. "
            "These tokens help maintain attention stability across long sequences. "
            "Default: 4")
        parser.add_argument(
            "--streaming-recent-size",
            type=int,
            default=EngineArgs.streaming_recent_size,
            help="Number of recent tokens to keep in the sliding window. "
            "This maintains the model's understanding of current context. "
            "Default: 512")
        parser.add_argument(
            "--streaming-enable-pos-shift",
            action="store_true",
            default=EngineArgs.streaming_enable_pos_shift,
            help="Enable position shift for rotary position embeddings. "
    ...
    def create_engine_config(
        ...
        streaming_config = StreamingConfig(
            ...
            enable_pos_shift=self.streaming_enable_pos_shift,
        )
